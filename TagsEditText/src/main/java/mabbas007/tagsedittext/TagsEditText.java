...
public class TagsEditText extends AutoCompleteTextView {
    ...
    public static final int TAG_ERROR_SPECIAL_CHARACTERS = 0;
    public static final int TAG_ERROR_EXISTS = 1;
    public static final int TAG_ERROR_SIZE_EXCEEDED = 2;
    ...
    private String mTagsPattern;
    private int mMaxTagsSize = -1;
    private boolean mAllTagsInSmallLetters = false;
    ...
    private void setTags() {
        ...
        if (mAllTagsInSmallLetters) {
            str = str.toLowerCase();
        }

        // In case done clicked
        ...
        if ((mLastString.endsWith(mSeparator) || mLastString.endsWith(NEW_LINE))
                && !isEnterClicked
                && isDeleting
                && !mTagSpans.isEmpty()) {
        ...
        // In case separator is clicked
        if (str.endsWith(NEW_LINE) || (!mIsSpacesAllowedInTags && str.endsWith(mSeparator)) &&
                !isDeleting && !mLastString.endsWith(mSeparator)) {

            buildTags(str);
        }

        // In case tags reached max size
        if (!str.endsWith(mSeparator) && !str.endsWith(NEW_LINE) &&
                mMaxTagsSize > 0 && getTags().size() >= mMaxTagsSize) {

            mListener.onError(TAG_ERROR_SIZE_EXCEEDED);
            StringBuilder strBuilder = new StringBuilder(str);
            strBuilder.setCharAt(str.length() - 1, mSeparator.toCharArray()[0]);
            str = strBuilder.toString();
            buildTags(str);
        }

        // In case character added not matching the patter
        if (mTagsPattern != null && !isDeleting && !isEnterClicked &&
                !str.isEmpty() && !str.endsWith(mSeparator)) {

            String addedCharStr = "";
            addedCharStr = String.valueOf(str.charAt(str.length() - 1));

            if (!addedCharStr.matches(mTagsPattern)) {
                mListener.onError(TAG_ERROR_SPECIAL_CHARACTERS);
                str = str.replace(addedCharStr, mSeparator);
                setTags(str.split(mSeparator));
            }
        }

        // In case a space is clicked while the last character was a space too
        if (!str.isEmpty() && mLastString.endsWith(mSeparator) && str.endsWith(mSeparator)) {
            setTags(str.split(mSeparator));
        }
    ...
    public void addTagsPattern(String pattern) {
        mTagsPattern = pattern;
    }

    public void addMaxTagsSize(int size) {
        mMaxTagsSize = size;
    }

    public void setAllTagsInSmallLetters(boolean value) {
        mAllTagsInSmallLetters = value;
    }
}
